...
class Barre:
    def __init__(self, window: pygame.Surface):
        ...
        self.image = pygame.image.load("images/barre.png").convert_alpha()
        self.pos = [400, 50]
        self.vect_dir = [-VITESSE_BARRE, VITESSE_BARRE]
    ...
    def move(self, dir: int = RIEN):
        if dir == HAUT:
            if self.pos[1] - VITESSE_BARRE >= 0:
                self.pos[1] -= VITESSE_BARRE
            else:
                self.pos[1] = 0
        if dir == BAS:
            if self.pos[1] + VITESSE_BARRE <= self.window_haut - self.image.get_height():
                self.pos[1] += VITESSE_BARRE
            else:
