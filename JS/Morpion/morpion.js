...
function jouer(event){
    document.getElementById(event.target.id).innerText = joueurs[joueur];
    document.getElementById(event.target.id).disabled = true;

    document.getElementById(event.target.id).classList.remove('btn-light');

    if (joueur == 0)
        document.getElementById(event.target.id).classList.add('btn-success');
    else
        document.getElementById(event.target.id).classList.add('btn-danger');
